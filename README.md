...
6. В отдельном терминале запустить консьюмер:
...
- Повторы вызова вебхука после успешного считывания события из очереди
- Для демонстрации этой функциональности, консюмер сознательно вызывает некорректный метод по адресу `https://example.com/webhook`
- Для упрощения структуры тестового проекта, неудачные попытки вызвать вебхук логируются, но не коммитятся в БД

## Проверки качества

Юнит-тесты без реальных интеграций на моках.
```bash
make test
```

Если необходим отчет покрытия
```bash
make test-cov
```

Линтер и статическая проверка типов на пре-коммит хуке.
```bash
make lint
```
Политика линтеров в `pyproject.toml`
